...
define(['jquery', 'data', 'homeLib/setcanvas', 'lib/jsonp'], function($, d, s, jsonp) {
	...
	return function(canvas, image) {
  		...
  		var canvasEvt = {
			...
			touch : ("ontouchstart" in document),
  			...
  			StartEvent : function(e) {
			    // var t = canvasEvt.touch ? e.touches[0] : e;//在触屏和鼠标都有的情况下的一个优化
		...
		var writeOpen = function(boolen) {
		  //canvas上书写的事件绑定函数
		  //在触屏和鼠标都有的情况下的一个优化 2017.02.16
		  opts.isWriteOpen = boolen;
		  var mouse = ("onmousedown" in document);
		  var touch = ("ontouchstart" in document);

		  var StartEvent = touch ? "touchstart" : '';
		  var MoveEvent = touch ? "touchmove" : '';
		  var EndEvent = touch ? "touchend" : '';

		  var StartEventMouse = mouse ? "mousedown" : '';
		  var MoveEventMouse = mouse ? "mousemove" : '';
		  var EndEventMouse = mouse ? "mouseup" : '';


		  if(boolen) {
			  StartEvent && canvas.addEventListener(StartEvent, canvasEvt.StartEvent, false);
			  MoveEvent && canvas.addEventListener(MoveEvent, canvasEvt.MoveEvent, false);
			  EndEvent && canvas.addEventListener(EndEvent, canvasEvt.EndEvent, false);
			  canvas.addEventListener("mouseout", canvasEvt.mouseout, false);
			  StartEventMouse && canvas.addEventListener(StartEventMouse, canvasEvt.StartEvent, false);
			  MoveEventMouse && canvas.addEventListener(MoveEventMouse, canvasEvt.MoveEvent, false);
			  EndEventMouse && canvas.addEventListener(EndEventMouse, canvasEvt.EndEvent, false);
		  } else {
			  StartEvent && canvas.removeEventListener(StartEvent, canvasEvt.StartEvent, false);
			  MoveEvent && canvas.removeEventListener(MoveEvent, canvasEvt.MoveEvent, false);
			  EndEvent && canvas.removeEventListener(EndEvent, canvasEvt.EndEvent, false);
			  canvas.removeEventListener("mouseout", canvasEvt.mouseout, false);
			  StartEventMouse && canvas.removeEventListener(StartEventMouse, canvasEvt.StartEvent, false);
			  MoveEventMouse && canvas.removeEventListener(MoveEventMouse, canvasEvt.MoveEvent, false);
			  EndEventMouse && canvas.removeEventListener(EndEventMouse, canvasEvt.EndEvent, false);
		  }
		};
